...
function UdhaarItems() {
  ...
  const handleSearch = async (e) => {
    e.preventDefault();
    if (!search.trim()) return fetchUdhaarItems();
    try {
      const res = await axios.get(`${API}/udhar-items/search/`, {
        params: { keyword: search },
        ...getAuthHeader()
      });
      setUdhaarItems(res.data || []);
      setCurrentPage(1);
    } catch (err) {
      const errorMsg = err.response?.data?.detail || err.response?.data?.error || "کوئی اُدھار آئٹم نہیں ملا";
      showMsg(errorMsg, "error");
    }
  };
  ...
  const currentItems = udhaarItems.slice(indexOfFirstItem, indexOfLastItem);
  const totalPages = Math.ceil(udhaarItems.length / itemsPerPage);
    ...
    <div className="min-h-screen bg-gray-50 p-3 md:p-6" dir="rtl">
      {message.text && (
        <div className={`fixed top-6 left-6 z-[100] px-6 py-3 rounded-2xl shadow-2xl animate-bounce transition-all text-sm md:text-base ${
          message.type === 'success' ? 'bg-amber-600 text-white' : 'bg-red-600 text-white'
        }`}>
          {message.text}
        </div>
      )}
      ...
      <div className="bg-white p-5 rounded-t-3xl shadow-sm border-b flex flex-col md:flex-row justify-between items-center gap-4">
        <div>
          <h2 className="text-2xl font-black text-gray-800">اُدھار آئٹمز</h2>
          <p className="text-gray-500 text-sm">کل اُدھار: {udhaarItems.length}</p>
        </div>
        ...
        <div className="flex gap-3 w-full md:w-auto">
          <form onSubmit={handleSearch} className="relative flex-1">
            <input 
              ...
              className="w-full md:w-80 p-4 pr-12 border-2 border-gray-200 rounded-3xl bg-gray-50 focus:border-amber-500 focus:bg-white outline-none transition-all text-base"
            />
          ...
          <button 
            ...
            className="bg-amber-600 hover:bg-amber-700 text-white px-8 py-4 rounded-3xl font-bold text-base transition-transform active:scale-95 whitespace-nowrap"
          >
            + نیا اُدھار
          </button>
        </div>
      ...
      <div className="bg-white shadow-xl rounded-b-3xl overflow-hidden border">
        {view === "LIST" ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-right">
                <thead className="bg-gray-100 border-b">
                  <tr>
                    <th className="p-5 border-l font-bold text-gray-700">کسٹمر</th>
                    <th className="p-5 border-l font-bold text-gray-700">آئٹم</th>
                    <th className="p-5 border-l font-bold text-gray-700 text-center">مقدار</th>
                    <th className="p-5 border-l font-bold text-gray-700 text-center">اکائی</th>
                    <th className="p-5 border-l font-bold text-gray-700 text-center">فی اکائی</th>
                    <th className="p-5 border-l font-bold text-gray-700 text-center">کل رقم</th>
                    <th className="p-5 border-l font-bold text-gray-700 text-center">تاریخ</th>
                    <th className="p-5 text-center font-bold text-gray-700">انتخاب</th>
                  </tr>
                </thead>
                <tbody>
                  {loading ? (
                    <tr><td colSpan="8" className="p-16 text-center text-gray-400">ڈیٹا لوڈ ہو رہا ہے...</td></tr>
                  ) : currentItems.length > 0 ? (
                    currentItems.map((item) => (
                      <tr key={item.udharitem_id} className="border-b hover:bg-amber-50/50 transition-colors">
                        <td className="p-5 border-l font-bold text-gray-800">{item.customer_name}</td>
                        <td className="p-5 border-l">{item.item_name}</td>
                        ...
                        <td className="p-5 border-l text-center font-mono text-amber-700 font-bold">{item.total_amount}</td>
                        <td className="p-5 border-l text-center text-sm text-gray-600">
                    ...
                    <tr><td colSpan="8" className="p-16 text-center text-gray-500">کوئی اُدھار آئٹم موجود نہیں ہے۔</td></tr>
                  )}
                </tbody>
              </table>
            </div>
                ...
                {[...Array(totalPages)].map((_, i) => (
                  <button
                    ...
                    className={`h-10 w-10 rounded-2xl border font-bold transition-all ${
                      currentPage === i + 1 ? "bg-amber-600 text-white shadow-inner" : "bg-white hover:bg-gray-100"
                    }`}
                  >
                    {i + 1}
                  </button>
          ...
          <UdhaarItemForm 
            ...
            onSave={() => { fetchUdhaarItems(); setView("LIST"); }}
            showMsg={showMsg}
          />
        )}
      </div>
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-[200] flex justify-center items-center p-4">
          <div className="bg-white p-8 rounded-3xl shadow-2xl max-w-sm w-full text-center">
            ...
            <p className="text-gray-500 mb-8">یہ اُدھار آئٹم ہمیشہ کے لیے حذف ہو جائے گا۔</p>
...
function UdhaarItemForm({ mode, initialData, onCancel, onSave, showMsg }) {
  ...
  // Prefill on Edit + handle custom unit
  useEffect(() => {
    if (initialData) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (mode === "ADD") {
        ...
        showMsg("اُدھار آئٹم کامیابی سے شامل کر دیا گیا", "success");
      } else if (initialData?.udharitem_id) {
        await axios.put(`${API}/udhar-items/${initialData.udharitem_id}`, payload, config);
        showMsg("اُدھار آئٹم کی تفصیلات اپ ڈیٹ ہو گئیں", "success");
      }
      onSave();
    } catch (err) {
      // ← This now shows the exact Urdu error from backend (e.g. unit mismatch)
  ...
  return (
    <div className="p-6 md:p-10 max-w-2xl mx-auto">
      <h3 className="text-3xl font-black mb-8 border-r-8 border-amber-600 pr-4">
        {mode === "ADD" ? "نیا اُدھار آئٹم شامل کریں" : "اُدھار آئٹم میں ترمیم"}
      </h3>

      <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-6 md:gap-8">
        <div>
          <label className="block text-sm font-black text-gray-700 mb-2">کسٹمر کا نام</label>
          <input 
            className={`w-full p-4 border-2 rounded-3xl outline-none text-base ${errors.customer_name ? 'border-red-500 bg-red-50' : 'border-gray-200 focus:border-amber-500'}`} 
        ...
        <div>
          ...
          <input 
            className={`w-full p-4 border-2 rounded-3xl outline-none text-base ${errors.item_name ? 'border-red-500 bg-red-50' : 'border-gray-200 focus:border-amber-500'}`} 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <select 
              className={`w-full p-4 border-2 rounded-3xl outline-none text-base ${errors.unit ? 'border-red-500' : 'border-gray-200 focus:border-amber-500'}`} 
              value={formData.unit} 
          ...
          {formData.unit === "__custom" && (
            <div>
              <label className="block text-sm font-black text-gray-700 mb-2">اپنی مرضی کی اکائی</label>
              <input 
                className="w-full p-4 border-2 border-gray-200 rounded-3xl text-base outline-none focus:border-amber-500" 
        ...
        <div>
          ...
          <input 
            type="number" 
            step="0.01"
            className={`w-full p-4 border-2 rounded-3xl outline-none text-base ${errors.quantity ? 'border-red-500 bg-red-50' : 'border-gray-200 focus:border-amber-500'}`} 
        ...
        <div className="flex gap-4 pt-4">
          <button 
            type="submit" 
            className="flex-1 bg-amber-600 text-white py-5 rounded-3xl font-black text-xl shadow-lg hover:bg-amber-700 active:scale-95 transition-all"
          >
            محفوظ کریں
          </button>
          <button 
